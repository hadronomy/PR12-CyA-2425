...
inline const k_stats fn(std::function<void()> fn, const k_options opts = k_options()) {
  k_stats stats;
  bool batch = false;
...
warmup: {
  ...
  if ((t1 - t0) <= opts.warmup_threshold) {
    for (auto o = 0; o < opts.warmup_samples; o++) {
      ...
      fn();
      const auto t1 = now();
      if ((batch = (t1 - t0) <= opts.batch_threshold))
        break;
    }
  }
}
...
inline const std::string barplot(std::map<std::string, f64> map, u64 legend = 8, u64 width = 14,
                                 bool colors = true) {
  std::string barplot = "";
...
inline const std::vector<std::string> histogram(k_bins bins, u64 height = 2, bool colors = true) {
  ...
  auto canvas = std::vector<std::vector<std::string>>(
      height, std::vector<std::string>(bins.bins.size(), " "));
...
inline const std::string boxplot(std::map<std::string, lib::k_stats> map, u64 legend = 8,
                                 u64 width = 14, bool colors = true) {
  std::string boxplot = "";
...
class B {
 ...
 public:
  ...
  B(std::string name, std::function<void()> fn) : name(name), fn(fn) {}
...
class runner {
 ...
 public:
  ...
  B* bench(const std::string name, std::function<void()> fn) {
  ...
  std::map<std::string, lib::k_stats> run(const k_run opts = k_run()) {
    ...
    if ("mitata" == opts.format) {
      ...
      for (const auto& collection : collections) {
        ...
        bool has_matches = std::any_of(
            collection.benchmarks.begin(), collection.benchmarks.end(), [&](const auto& bench) {
        ...
        for (const auto& [name, bench] : collection.benchmarks) {
          ...
          if (compact) {
            ...
            if (!opts.colors)
              ...
              std::cout << fmt::colors::bold << fmt::colors::yellow << avg << fmt::colors::reset
                        << fmt::colors::bold << "/iter" << fmt::colors::reset;
            ...
            if (!opts.colors)
              ...
              std::cout << fmt::colors::bold << fmt::colors::yellow << avg << fmt::colors::reset
                        << fmt::colors::bold << "/iter" << fmt::colors::reset << " "
                        << fmt::colors::gray << p75 << fmt::colors::reset << " " << histogram[0];
            ...
            if (!opts.colors)
              ...
              std::cout << fmt::colors::cyan << min << fmt::colors::reset << fmt::colors::gray
                        ...
                        << fmt::colors::reset << fmt::colors::gray << ")" << fmt::colors::reset;

            std::cout << " ";
            if (!opts.colors)
              std::cout << p99 << " " << histogram[1];
            else
              std::cout << fmt::colors::gray << p99 << fmt::colors::reset << " " << histogram[1];
          }
        ...
        if (collection.types.end() !=
          ...
          auto baseline = std::find_if(
              trials.begin(), trials.end(), [](const auto& trial) { return trial.second.first; });

          std::cout << std::endl;
          if (baseline == trials.end())
            baseline = trials.begin();
          if (!opts.colors)
          ...
          if (!opts.colors)
            std::cout << "  " << baseline->first << std::endl;
          else
            std::cout << "  " << fmt::colors::bold << fmt::colors::cyan << baseline->first
                      << fmt::colors::reset << std::endl;
          ...
          for (const auto trial : trials) {
            ...
            auto diff = !faster ? std::round(1 / c.avg * b.avg * 100) / 100
